...
DESCRIPTION
  ...
  validation {
    condition     = var.entity_type != "environment" || (var.entity_type == "environment" && length(tostring(var.environment_name)) > 0)
...
DESCRIPTION
  ...
  validation {
    condition     = var.entity_type != "branch" || (var.entity_type == "branch" && length(tostring(var.branch_name)) > 0)
...
DESCRIPTION
  ...
  validation {
    condition     = var.entity_type != "tag" || (var.entity_type == "tag" && length(tostring(var.tag_name)) > 0)
